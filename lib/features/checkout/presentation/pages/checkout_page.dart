...
class _CheckoutPageState extends ConsumerState<CheckoutPage> {
  ...
  Future<void> _submit(List<CartItem> items, double total) async {
    ...
    // La compra definitiva es una operación crítica (sección 19 del
    // documento): se revisa la conectividad antes de intentarla en lugar
    // de esperar a que la llamada falle para avisar.
    if (!await ref.read(connectivityServiceProvider).isOnline()) {
      messenger
        ..removeCurrentSnackBar()
        ..showSnackBar(
          const SnackBar(content: Text('Sin conexión a internet. Conéctate para completar la compra.')),
        );
      return;
    }
...
class _PaymentSectionState extends ConsumerState<_PaymentSection> {
  ...
  Future<void> _pay() async {
    ...
    // El pago es una operación crítica (sección 19 del documento): se
    // revisa la conectividad antes de intentarlo, tanto para tokenizar
    // la tarjeta con Stripe como para confirmar el pago en el backend.
    if (!await ref.read(connectivityServiceProvider).isOnline()) {
      messenger
        ..removeCurrentSnackBar()
        ..showSnackBar(
          const SnackBar(content: Text('Sin conexión a internet. Conéctate para completar el pago.')),
        );
      return;
    }
